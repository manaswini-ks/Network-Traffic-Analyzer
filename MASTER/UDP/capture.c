...
void packet_handler(const u_char *packet, size_t packet_len, struct pcap_pkthdr *pkthdr) {
    ...
    if (ip_header->protocol == IPPROTO_UDP) {
        ...
        // Determine the validity of the packet based on checksum comparison
        const char *validity = (src_checksum == dest_checksum) ? "Valid" : "Invalid";
        printf("Packet Integrity: %s\n", validity);
        ...
        if (!headings_printed) { // Check if the headings have been printed
            fprintf(data_csv_file, "Packet Number,Timestamp,Relative Time,Source IP,Source Port,Destination IP,Destination Port,IP Protocol,Frame Length,Packet Size,Payload Size,Src Checksum,Dest Checksum,Validity,TTL\n");
        ...
        // Write the packet data to CSV with the additional columns for dest checksum and validity
        fprintf(data_csv_file, "%u,%s,%lf,%s,%u,%s,%u,%u,%u,%zu,%d,0x%04x,0x%04x,%s,%u\n",
                ...
                src_checksum, // Source checksum
                dest_checksum, // Destination checksum
                validity, // Validity
                ip_header->ttl); // TTL
