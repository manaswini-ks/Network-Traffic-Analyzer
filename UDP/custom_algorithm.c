...
void custom_algorithm(uint8_t tcp_flags, const char *src_ip, uint16_t src_port, const char *dst_ip, uint16_t dst_port, uint8_t ip_proto, double time_relative, uint32_t frame_len, char *classification, size_t classification_size)
{
    ...
    // Write to text file
    write_to_txt(src_ip, src_port, dst_ip, dst_port, ip_proto, tcp_flags, time_relative, frame_len, classification);
